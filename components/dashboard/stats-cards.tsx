...
async function getStats() {
  ...
  if (!process.env.MONGODB_URI) {
    console.log('No MONGODB_URI configured, using demo data');
    return DEMO_STATS;
  }
  ...
  try {
    ...
    const [
      ...
      Cluster.countDocuments({ priority: 'critical', status: { $ne: 'resolved' } }),
    ]);
...
export async function StatsCards() {
  ...
  return (
    <div className="space-y-4">
      {stats.isDemo && (
        <div className="rounded-lg border border-amber-500/50 bg-amber-500/10 p-4 text-center">
          <p className="text-sm text-amber-700 dark:text-amber-400">
            <strong>Demo Mode:</strong> Showing sample data. Connect MongoDB and add API keys in <code className="rounded bg-amber-500/20 px-1">.env.local</code> for live data.
          </p>
        </div>
      )}
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Issue Clusters</CardTitle>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="none"
      ...
      <Card className={stats.criticalClusters > 0 ? 'border-red-500/50 pulse-critical' : ''}>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <CardContent>
          <div className={`text-2xl font-bold ${stats.criticalClusters > 0 ? 'text-red-500' : ''}`}>
